...
from ui.components import leftover_input_csv, leftover_input_manual, leftover_input_firebase
from ui.components import render_auth_ui, initialize_session_state, auth_required, get_current_user, is_user_role
from ui.components import render_cooking_quiz, display_gamification_dashboard, award_recipe_generation_xp, display_daily_challenge, show_xp_notification
...
#this is to find the features that the users cant access so that we can show that in sidebar 
    all_features = {
    ...
    if inaccessible:
        return "**Features you are not able to  access:** " + ", ".join(inaccessible)
    else:
        return "**You have access to all features.**"

@auth_required # only logged in users can access if this is used. 
            ...
            with st.expander("Loaded Ingredients", expanded=False):
                if st.session_state.detailed_ingredient_info:
                    for item in st.session_state.detailed_ingredient_info:
                        ...
                        # color coding! error - red, about to expire very soon. warning - yellow, expiring soon. green - expiring later 
                        if days_left <= 1:
...
def ingredients_management():
    if render_ingredient_management:
        ...
        st.error("The ingredients management feature is not available.")
...
def gamification_hub():
    ...
    if user and user.get('user_id'):
        try:
            ...
            st.markdown("Stats temporarily unavailable..")
    else:
...
def main():
    ...
    if selected_feature == "Dashboard":
        ...
        visual_menu_search()
# reminder for self - all done as of 14/6/25.
