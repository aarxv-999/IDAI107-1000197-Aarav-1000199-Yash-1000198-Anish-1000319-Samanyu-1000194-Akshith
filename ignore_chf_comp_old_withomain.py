...
def render_menu_generator(db):
    st.markdown("### Weekly Menu Generator")
    ...
    if generate_button:
        ...
        if existing_docs:
            st.warning("A menu for this week has already been generated.")
        ...
        for doc in ingredients_docs:
            ...
            if expiry_date:
                try:
                    ...
                    days_to_expiry = (expiry_dt.replace(tzinfo=None) - datetime.now()).days
                    ...
                    logger.error(f"Error parsing expiry date for ingredient: {e}")
                    continue
...
def confirm_menu_replacement(db, start_of_week):
    st.error("WARNING: This will delete the current week's menu and generate a new one.")
