...
def render_auth_ui():
    ...
    if st.session_state.is_authenticated:
        ...
        selected_feature = st.sidebar.selectbox(
            "Choose a feature:",
            features,
            index=0 if 'selected_feature' not in st.session_state else features.index(st.session_state.get('selected_feature', 'Dashboard')),
            key="feature_selector"
        )

        # Update session state when selection changes
        if selected_feature != st.session_state.get('selected_feature'):
