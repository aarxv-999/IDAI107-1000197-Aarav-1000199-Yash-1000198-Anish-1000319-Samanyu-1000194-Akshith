...
def get_inaccessible_features_message(user_role):
    all_features = {
        ...
        'Promotion Generator': 'Available to all users',
...
def leftover_management():
    ...
    if not check_feature_access("Leftover Management"):
        st.error("Access denied. This feature is only available to Staff, Chef, and Admin users.")
        return
    ...
    if st.session_state.leftover_method is None:
        ...
        if st.session_state.leftover_method == "manual":
            ...
            st.info("The CSV must have a column named 'ingredient' with all leftover ingredients.")
...
def ingredients_management():
    # Check access before rendering
    if not check_feature_access("Ingredients Management"):
        st.error("Access denied. This feature is only available to Staff, Chef, and Admin users.")
        return
...
def chef_recipe_suggestions():
    # Check access before rendering
    if not check_feature_access("Chef Recipe Suggestions"):
        st.error("Access denied. This feature is only available to Chef and Admin users.")
        return
...
def main():
    ...
    # Create navigation buttons in sidebar for available features only
    st.sidebar.title("Navigation")
    
    for feature in available_features:
        if st.sidebar.button(feature, key=f"nav_{feature}", use_container_width=True):
            st.session_state.selected_feature = feature
            st.rerun()

    # Get current selected feature
    selected_feature = st.session_state.get('selected_feature', 'Dashboard')
    
    # Ensure selected feature is accessible
    if selected_feature not in available_features:
        selected_feature = "Dashboard"
        st.session_state.selected_feature = selected_feature

    # Show current page indicator
    st.sidebar.markdown(f"**Current Page:** {selected_feature}")
